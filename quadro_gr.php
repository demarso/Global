...
include "conexao.php";
  /*        
     ...
     if($nomer == "ITAGUAÍ"){
      ...
      $nomee = "PLANTAO GLOBAL - NOVA IGUAÇU";
      $sql0 = "SELECT idEquipe, nome, status FROM equipe WHERE nome = '$nomee' AND status = 'Ativo'";
...
<br /><br />
 <center>
  <H2><font color="blue"><b>EQUIPE <? echo $nomee;  ?></b></font></H2>
...
if($equipe1 == $nomee){
  /* if($mes == $mes_atual && $ano == $ano_atual){
      ...
      $sql = "SELECT idConsultor, count(idConsultor) as cons FROM producao WHERE idConsultor='$id' AND MONTH(dataProposta) = '$mes' AND YEAR(dataProposta) = '$ano'";
  // }
//$sql = "SELECT idConsultor, count(idConsultor) as cons FROM producao WHERE idConsultor='$id' AND MONTH(dataProposta) = '$mes' AND YEAR(dataProposta) = '$ano'  AND status = 'Ativo'";
